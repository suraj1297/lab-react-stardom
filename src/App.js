...
export default class App extends Component {
    ...
    componentDidMount() {
        this.setState({ stars: { ...this.getTopFour() } })
    }
    ...
    getRandomStars = () => {
        ...
        let starsList = {}
        let array = [1, 1, 1, 1].map(() => { return Math.floor(Math.random() * prostars.length) })
        ...
        return (
            <React.Fragment>
                <div className="buttons">
                    <div className="random" onClick={this.getRandomStars}>Get Random Contact</div>
                    <div className="byName" onClick={this.getByName}>Get By Name</div>
                    <div className="byPopularity" onClick={this.getByPopularity}>Get By Popularity</div>
                </div>
                <table>
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        {
                            ids.map(id =>
                                <StarRow key={id} {...stars[id]} id={id} deleteStar={this.deleteStar} />
                            )
